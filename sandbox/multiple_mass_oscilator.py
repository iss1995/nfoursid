...
from sympy import EX, N, stationary_points
...
def construct_state_space(
    n: int, Ms: List[int], Ks: List[int], Cs: List[int], L: int = 1, dt: float = 0.1
    ...
    return StateSpace(A * dt, B * dt, C, D), F
...
phase = np.linspace(0, 2 * np.pi, EXTRAPOLATION_LENGTH)
FORCING = (0.5 * np.sin(10 * phase) + 0.5 * np.cos(phase)) * sigmoid(np.linspace(0, EXTRAPOLATION_LENGTH*DT, EXTRAPOLATION_LENGTH))

ss, F = construct_state_space(n, Ms, Ks, Cs, L, DT)
# %% check stability
...
fig, axs = plt.subplots(2, 1, figsize=(12, 8))
for i in range(n):
    axs[0].plot(y[:, i], label=f"mass {i+1}")
axs[1].plot(FORCING, label="forcing")
